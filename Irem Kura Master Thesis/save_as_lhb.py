...
try:
    ...
    for index, filename in enumerate(filenames):
        ...
        if r'xray2\P' in str(one_up):
            categories.append(1)
        elif r'xray2\C' in str(one_up):
            categories.append(0)
except:
...
lab = cv2.imread('train_CT1/COVID2_CT/5d5d5 (6).jpg')
lab_23 = cv2.cvtColor(lab, cv2.COLOR_BGR2LAB)   

lab_hsv_bgr = cv2.cvtColor(lab_23, cv2.COLOR_HSV2BGR)
...
for index, row in data_df.iterrows():
    ...
    t_img = cv2.cvtColor(temp_img, cv2.COLOR_BGR2LAB)
    img = cv2.cvtColor(t_img, cv2.COLOR_HSV2BGR)
    ...
    if not os.path.exists("labHB\\"+ whole_left_dir):
        os.makedirs("labHB\\"+ whole_left_dir)
    cv2.imwrite("labHB\\" + row['filenames'], img)
