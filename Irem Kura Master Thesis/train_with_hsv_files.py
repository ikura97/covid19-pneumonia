...
try:
    filenames = glob(r"hsv\yeni_datasetler\xray1" + "\PNEUMONIA\*.*")
    filenames += glob(r"hsv\yeni_datasetler\xray1" + "\COVID19\*\*.*")

    categories = []
    for index, filename in enumerate(filenames):
        one_up = Path(filename).resolve().parents[0]
        if r'hsv\yeni_datasetler\xray1\PNEUMONIA' in str(one_up):
            categories.append(1)
        elif r'hsv\yeni_datasetler\xray1\COVID19' in str(one_up):
            categories.append(0)
except:
...
IMAGE_HEIGHT = 150
IMAGE_CHANNELS = 3
version_str = "xray1_hsv_all_tryout"
#version_str = "tao_hsv_all_tryout"
#version_str = "ct1_hsv_all_tryout"
...
with open('xray1_hsv_model_from_project_helper_' + version_str + month_day_hour_extractor('Europe/Istanbul') + '{}.json', "w") as json_file:
...
mc = ModelCheckpoint('xray1_hsv_model_from_project_helper_' + version_str + month_day_hour_extractor('Europe/Istanbul') +
...
try:
    ...
    ax1.figure.savefig('Hsv_ Acc & Loss_project_helper_' + version_str +
...
try:
    ...
    total = sum(sum(conf))
    # from conf. matrix calculate accuracy
