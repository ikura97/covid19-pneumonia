...
IMAGE_CHANNELS = 3
version_str = "CT1_kfold_all_tryout"
batch_size = 64
epochs = 20
...
try:
    filenames = glob("labHB\yeni_datasetler\ct1" + "\pneumonia_CT\*.*")
    filenames += glob("labHB\yeni_datasetler\ct1" + "\COVID2_CT\*.*")

    categories = []
    for index, filename in enumerate(filenames):
        one_up = Path(filename).resolve().parents[0]
        if r'labHB\yeni_datasetler\ct1\pneumonia_CT' in str(one_up):
            categories.append(1)
        elif r'labHB\yeni_datasetler\ct1\COVID2_CT' in str(one_up):
            categories.append(0)
except:
...
for train_idx, test_idx in kf.split(X, y):
    ...
    with open('kfold_tryout_' + version_str + month_day_hour_extractor('Europe/Istanbul') + '{}.json', "w") as json_file:
    ...
    mc = ModelCheckpoint('labHB2_model_from_project_helper_' + version_str + month_day_hour_extractor('Europe/Istanbul') +
    ...
    history = model.fit(
        ...
        steps_per_epoch=nb_samples_of_train//batch_size,  
        callbacks=callbacks
    )
    ...
    # k fold için her bir metrik için listeye append ediyoruz sonuçları
