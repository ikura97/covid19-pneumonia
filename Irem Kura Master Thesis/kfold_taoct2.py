...
# yeni nesil metrics için import
...
IMAGE_CHANNELS = 3
version_str = "CT2TAO_kfold_all_tryout"
batch_size = 64
epochs = 20
...
try:
    filenames = glob(r"labHB\yeni_datasetler\ct2" + "\CP5\*.*")
    filenames += glob(r"labHB\yeni_datasetler\ct2" + "\COVID19\*\*.*")

    categories = []
    for index, filename in enumerate(filenames):
        one_up = Path(filename).resolve().parents[0]
        if r'labHB\yeni_datasetler\ct2\CP5' in str(one_up):
            categories.append(1)
        elif r'labHB\yeni_datasetler\ct2\COVID19' in str(one_up):
            categories.append(0)
except:
...
for train_idx, test_idx in kf.split(X, y):
    ...
    with open('kfold_tryout_CT2TAO' + version_str + month_day_hour_extractor('Europe/Istanbul') + '{}.json', "w") as json_file:
    ...
    mc = ModelCheckpoint('labHB2_ct2_model_from_project_helper_' + version_str + month_day_hour_extractor('Europe/Istanbul') +
