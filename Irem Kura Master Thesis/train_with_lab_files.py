...
IMAGE_CHANNELS = 3
#version_str = "xray1_lab_all_tryout"
#version_str = "c1_lab_all_tryout"
#version_str = "tao_lab_all_tryout"
version_str = "xray2_lab_all_tryout"
batch_size = 64
...
try:
    filenames = glob(r"lab\yeni_datasetler\xray2" + r"\P\*.*")
    filenames += glob(r"lab\yeni_datasetler\xray2" + r"\C\*.*")

    categories = []
    for index, filename in enumerate(filenames):
        one_up = Path(filename).resolve().parents[0]
        if r'lab\yeni_datasetler\xray2\P' in str(one_up):
            categories.append(1)
        elif r'lab\yeni_datasetler\xray2\C' in str(one_up):
            categories.append(0)
except:
...
with open('x2_lab2_model_from_project_helper_' + version_str + month_day_hour_extractor('Europe/Istanbul') + '{}.json', "w") as json_file:
...
mc = ModelCheckpoint('x2_lab2_model_from_project_helper_' + version_str + month_day_hour_extractor('Europe/Istanbul') +
...
# ♦history = fit_model(model, train_generator, nb_samples_of_train, validation_generator, nb_samples_of_val, batch_size, callbacks, train_df, validate_df, epochs)

# Visualization starts
#version_for_experiment = "lab2_ct_tryout"
#version_for_experiment = "lab2_tao_all_tryout"
version_for_experiment = "lab2_x2_tryout"
...
try:
    ...
    # max_for_loss is for loss' y_ticks's max value to visualize.
    max_for_loss = 0
    ...
    # max_for_acc is for accs' y_ticks's max value to visualize.
    max_for_acc = 0
    ...
    # min_for_acc = np.amin(history.history['val_accuracy'])
    ...
    ax1.figure.savefig('lab2_x2_ Acc & Loss_project_helper_' + version_str +
...
df = metrics
arr = jaccard
desired_file_name = version_for_experiment + '_metrics'
